...
Note that while the trainable adjacency matrix $A _ {\text{bias}}$ involves computing $E _ 1 E _ 2 ^ T$ (complexity $O(N ^ 2 C)$), this matrix is precomputed during training and stored as a fixed parameter. During inference, $A _ {\text{bias}}$ is simply loaded and used, contributing only $O(N C)$ complexity for the multiplication $\kappa(A _ {\text{bias}})V$. Therefore, the overall inference complexity of CMI remains $O(N C ^ 2)$.
